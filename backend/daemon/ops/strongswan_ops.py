...
def _load_peer_name_map() -> dict[str, int]:
    ...
    try:
        ...
        return {peer.name: peer.peerId for peer in peers}
    finally:
        session.close()
...
def write_peer_config(
    ...
    *,
    ...
    config_file = conf_path / f"{name}.conf"
...
def initiate_peer(
    ...
    *,
    ...
    try:
        result = runner(
            ["swanctl", "--initiate", "--child", f"{name}-child"],
            capture_output=True,
            text=True,
            timeout=5,
        )
...
def teardown_peer(
    ...
    *,
    ...
    try:
        result = runner(
            ["swanctl", "--terminate", "--child", f"{name}-child"],
            capture_output=True,
            text=True,
            timeout=5,
        )
...
def remove_peer_config(
    ...
    *,
    ...
    Returns:
        Status dict with result details.
    """
    config_file = Path(conf_dir) / f"{name}.conf"
...
def write_routes_config(
    ...
    *,
    ...
    """Update a peer's strongSwan config to include route traffic selectors.

    Reads the existing peer config file, updates the local_ts directive
    with comma-separated CIDRs from the routes list.
    ...
    cidrs = ",".join(r["destination_cidr"] for r in routes)
    if not cidrs:
        cidrs = "0.0.0.0/0"
    ...
    try:
        ...
        # Update or add local_ts in the children block
        if "local_ts" in content:
            # Replace existing local_ts line
            lines = content.split("\n")
            new_lines = []
            for line in lines:
                if "local_ts" in line and "=" in line:
                    indent = line[: len(line) - len(line.lstrip())]
                    new_lines.append(f"{indent}local_ts = {cidrs}")
                else:
                    new_lines.append(line)
            content = "\n".join(new_lines)
        else:
            # Insert local_ts before remote_ts or before esp_proposals
            lines = content.split("\n")
            new_lines = []
            inserted = False
            for line in lines:
                if not inserted and ("remote_ts" in line or "esp_proposals" in line or "mode = tunnel" in line):
                    indent = line[: len(line) - len(line.lstrip())]
                    new_lines.append(f"{indent}local_ts = {cidrs}")
                    inserted = True
                new_lines.append(line)
        ...
        logger.info(f"Updated routes for peer {name}: {cidrs}")
...
def configure_peer(
    ...
    *,
    ...
    Generates configuration, validates syntax, and writes to conf.d.
